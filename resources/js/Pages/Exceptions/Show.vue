...
export default {
    ...
    methods: {
        fixed() {
            this.$inertia.post(this.route('exceptions.fixed', [this.project.id, this.exception.id]));
        },
        snooze(minutes) {
            this.$inertia.post(this.route('exceptions.snooze', [this.project.id, this.exception.id]), {
                snooze: minutes
            });
        },
        throwInTrash() {
            this.$inertia.delete(this.route('exceptions.show', [this.project.id, this.exception.id]));
        },
        togglePublic() {
            this.$inertia.post(this.route('exceptions.toggle-public', [this.project.id, this.exception.id]));
        },
        unSnooze() {
            this.$inertia.post(this.route('exceptions.un-snooze', [this.project.id, this.exception.id]));
        },
    }
}
</script>
    ...
    <AppLayout title="Project Exception">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex w-full bg-white px-6 py-3 border-b border-gray-200 space-x-3">
                ...
                <Button as="InertiaLink" :href="exception.project_route_url" secondary>Return to Project</Button>
